...
class KafkaManager(akkaConfig: Config) extends Logging {
  ...
  {
    implicit val ec = apiExecutionContext
    withKafkaManagerActor(KMClusterCommandRequest(clusterName, CMDeleteTopicc(offsetPath,offset))) {
      result: Future[CMCommandResult] =>
        result.map(cmr => toDisjunction(cmr.result))
    }
  }
  def delConsumerGroup(
                        clusterName: String,
                        consumerGroup:String
                      ): Future[ApiError \/ ClusterContext] =
  {
    implicit val ec = apiExecutionContext
    withKafkaManagerActor(KMClusterCommandRequest(clusterName, CMDelConsumerGroup(consumerGroup))) {
      result: Future[CMCommandResult] =>
        result.map(cmr => toDisjunction(cmr.result))
    }
  }
  def resetConsumerOffset(
                           clusterName: String,
                           consumerGroup:String,
                           topic: String,
                           topicInfo: String
  ...
  {
    implicit val ec = apiExecutionContext
    withKafkaManagerActor(KMClusterCommandRequest(clusterName, CMResetConsumerGroup(consumerGroup,topic,topicInfo))) {
      result: Future[CMCommandResult] =>
        result.map(cmr => toDisjunction(cmr.result))
    }
  }
