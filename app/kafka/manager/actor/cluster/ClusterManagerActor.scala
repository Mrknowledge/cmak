...
class ClusterManagerActor(cmConfig: ClusterManagerActorConfig)
  ...
  override def processCommandRequest(request: CommandRequest): Unit = {
    request match {
      ...
      //ken add
      case CMDeleteTopicc(offsetPath,offset) =>
        implicit val ec = longRunningExecutionContext
        withKafkaCommandActor(KCDeleteTopicc(offsetPath,offset)) {
          kcResponse: KCCommandResult =>
            Future.successful(CMCommandResult(kcResponse.result))
        } pipeTo sender()
      case CMDelConsumerGroup(consumerGroup) =>
        implicit val ec = longRunningExecutionContext
        withKafkaCommandActor(KCDelConsumerGroup(consumerGroup)) {
          kcResponse: KCCommandResult =>
            Future.successful(CMCommandResult(kcResponse.result))
        } pipeTo sender()
      case CMResetConsumerGroup(consumerGroup,topic,topicInfo) =>
        implicit val ec = longRunningExecutionContext
        withKafkaCommandActor(KCResetConsumerGroup(consumerGroup,topic,topicInfo)) {
          kcResponse: KCCommandResult =>
            Future.successful(CMCommandResult(kcResponse.result))
        } pipeTo sender()
