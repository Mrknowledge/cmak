...
class KafkaCommandActor(kafkaCommandActorConfig: KafkaCommandActorConfig) extends BaseClusterCommandActor with LongRunningPoolActor {
  ...
  override def processCommandRequest(request: CommandRequest): Unit = {
    ...
    request match {
      ...
      //ken add
      case KCDeleteTopicc(offsetPath,offset) =>
        featureGateFold(KMDeleteTopicFeature)(
          {
            val result : KCCommandResult = KCCommandResult(Failure(new UnsupportedOperationException(
              s"Delete topic not supported for kafka version ${kafkaCommandActorConfig.clusterContext.config.version}")))
          ...
          },
          {
            longRunning {
              Future {
                KCCommandResult(Try {
                  //                log.info(s"Deleting topic : $topic")
                  kafkaCommandActorConfig.adminUtils.updateOffset(kafkaCommandActorConfig.curator, offsetPath,offset)
                })
              }
            }
          })
      case KCDelConsumerGroup(consumergroup) =>
        featureGateFold(KMDeleteTopicFeature)(
          {
            val result : KCCommandResult = KCCommandResult(Failure(new UnsupportedOperationException(
              s"Delete topic not supported for kafka version ${kafkaCommandActorConfig.clusterContext.config.version}")))
          ...
          },
          {
            longRunning {
              Future {
                KCCommandResult(Try {
                  //                log.info(s"Deleting topic : $topic")
                  kafkaCommandActorConfig.adminUtils.delConsumerGroup(kafkaCommandActorConfig.curator, consumergroup)
                })
              }
            }
          })
      case KCResetConsumerGroup(consumergroup,topic,topicInfo) =>
        featureGateFold(KMDeleteTopicFeature)(
          {
            val result : KCCommandResult = KCCommandResult(Failure(new UnsupportedOperationException(
              s"Delete topic not supported for kafka version ${kafkaCommandActorConfig.clusterContext.config.version}")))
          ...
          },
          {
            longRunning {
              Future {
                KCCommandResult(Try {
                  //                log.info(s"Deleting topic : $topic")
                  kafkaCommandActorConfig.adminUtils.resetConsumerGroup(kafkaCommandActorConfig.curator, consumergroup,topic,topicInfo)
                })
              }
            }
          })
