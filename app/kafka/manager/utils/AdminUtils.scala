...
class AdminUtils(version: KafkaVersion) extends Logging {
  ...
  //ken add
  def updateOffset(curator: CuratorFramework, offsetPath: String,offset:Long): Unit = {
    //    ZkUtils.updateOffsetByPartition(curator,"/consumers/kafka-node-group/offsets/test/0")
    ZkUtils.updateOffsetByPartition(curator, offsetPath, offset)
  }
  def delConsumerGroup(curator: CuratorFramework, consumerGroup: String): Unit = {
    ZkUtils.delConsumerGroup(curator, consumerGroup)
  }
  def resetConsumerGroup(curator: CuratorFramework, consumerGroup: String, topic: String, topicInfo: String): Unit = {
    ZkUtils.resetConsumerGroup(curator, consumerGroup, topic, topicInfo )
  }
